/* $Header: /usr/people/sam/fax/libtiff/RCS/tif_aux.c,v 1.20 1994/05/16 18:52:55 sam Exp $ */
...
static void
DECLARE1(TIFFDefaultTransferFunction, TIFFDirectory*, td)
{
	u_short **tf = td->td_transferfunction;
	int i, n = 1<<td->td_bitspersample;

	tf[0] = (u_short *)_TIFFmalloc(n * sizeof (u_short));
	tf[0][0] = 0;
	for (i = 1; i < n; i++)
		tf[0][i] = (u_short)floor(65535.*pow(i/(n-1.), 2.2) + .5);
	if (td->td_samplesperpixel - td->td_matteing > 1) {
		tf[1] = (u_short *)_TIFFmalloc(n * sizeof (u_short));
		memcpy(tf[1], tf[0], n * sizeof (u_short));
		tf[2] = (u_short *)_TIFFmalloc(n * sizeof (u_short));
		memcpy(tf[2], tf[0], n * sizeof (u_short));
	}
}

static void
DECLARE1(TIFFDefaultRefBlackWhite, TIFFDirectory*, td)
{
	int i;
