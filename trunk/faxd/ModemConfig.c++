...
#if defined(PHOTOMETRIC_ITULAB) || ( defined(HAVE_JPEG) && ( defined(HAVE_LCMS) || defined(HAVE_LCMS2) ) )
...
ModemConfig::setConfigItem(const char* tag, const char* value)
{
	...
	if (tag[6] == 'p') {
		...
		switch (tag[6]) {
			...
			break;
		    case 'r':
			idConfig[callidIndex].record = getBoolean(value);
			configTrace("CallID[%d].record = %s", callidIndex,
			    idConfig[callidIndex].record ? "true" : "false");
			break;
		    case 'l':
