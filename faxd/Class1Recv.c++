/*	$Id: Class1Recv.c++ 293 2006-09-07 18:59:29Z faxguy $ */
/*
...
Class1Modem::recvPage(TIFF* tif, u_int& ppm, fxStr& emsg, const fxStr& id)
{
	...
	if (!messageReceived) {
		...
		if (rmResponse == AT_CONNECT) {
			...
			if (messageReceived)
			    prevPage++;
		        timer = (64*1024*8) / ((curcap->br+1)*2400) * 1000;	// wait longer for PPM (estimate 64KB)
		    }
		} else {
		    if (wasTimeout()) {
		    ...
		    bool getframe = false;
		    long wait = (64*1024*8) / ((curcap->br+1)*2400) * 1000;
