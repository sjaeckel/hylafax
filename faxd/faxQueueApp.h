/*	$Header: /usr/people/sam/fax/./faxd/RCS/faxQueueApp.h,v 1.26 1995/04/08 21:31:19 sam Rel $ */
/*
...
#define	NQHASH		16
    ...
    static const fxStr fifoName;	// server FIFO name
...
// modem support
    void	modemsOnCmdLine(int argc, char**argv);
    void	scanForModems();
    void	releaseModem(Job& job);
// diagnostic and tracing interfaces
    void	jobError(Job& job, const char* fmt ...);
    void	traceQueue(Job&, const char* fmt ...);
    void	traceJob(Job&, const char* fmt ...);
...
// qfile request stuff
    ...
    void	deleteRequest(Job&, FaxRequest& req, JobStatus why,
		    fxBool force, const char* duration = "");
    void	notifySender(const fxStr&, Job&, JobStatus, const char* = "");
// job management interfaces
    ...
    void	alterJob(const char*);
    void	submitJob(const fxStr& filename);
    void	rejectSubmission(Job&, FaxRequest&, const fxStr& reason);
    Job*	activeJob(const fxStr& filename);

    void	setReadyToRun(Job& job);
    void	setSleep(Job& job, time_t tts);
    void	setDead(Job& job);
    void	setActive(Job& job);
    void	submitJob(const fxStr& file, FaxRequest&);
    void	terminateJob(const fxStr& filename, JobStatus how);
    void	terminateJob(Job& job, JobStatus why);
    void	timeoutJob(Job& job);
    fxBool	changeKillTime(Job& job, time_t kt);
    void	runJob(Job& job);
...
// job preparation stuff
    ...
    JobStatus	convertPostScript(Job&,
		    const fxStr& inFile, const fxStr& outFile,
		    const Class2Params& params, fxStr& emsg);
    JobStatus	runPostScript(Job& job, const fxStr& cmd, char* const* argv,
		    fxStr& emsg);
    fxBool	runPostScript1(Job& job, int fd, fxStr& output);
    JobStatus	checkFileFormat(Job&,
		    const fxStr& file, const FaxMachineInfo& info, fxStr& emsg);
    fxBool	checkPageFormat(Job&, TIFF*,
